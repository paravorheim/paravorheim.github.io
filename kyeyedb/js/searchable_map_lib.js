...
var SearchableMapLib = {
  ...
  initialize: function(options){
    ...
    SearchableMapLib.map_centroid = options.map_centroid || [38.0304861, -84.5103392],
    SearchableMapLib.defaultZoom = options.defaultZoom || 7,
  ...
  createSQL: function() {
    ...
    if (name_search != '') {
      SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
          return (r.properties["First Name"] + " " +  r.properties["Last Name"]).toLowerCase().indexOf(name_search.toLowerCase()) > -1;
        });
    }
  ...
  addIcon: function() {
    SearchableMapLib.centerMark = new L.Marker(SearchableMapLib.currentPinpoint, { icon: (new L.Icon({
            iconUrl: '../kyeyedb/img/blue-pushpin.png',
            iconSize: [32, 32],
            iconAnchor: [10, 32]
    }))});
