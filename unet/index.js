...
//   const mobilenet = await tf.loadLayersModel(tf.io.browserFiles(
//     [uploadJSONInput.files[0], uploadWeightsInput.files[0]]));
  // const handler = tfnode.io.fileSystem(MOBILENET_MODEL_PATH);
  mobilenet = await tf.loadLayersModel(MOBILENET_MODEL_PATH);

  // Warmup the model. This isn't necessary, but makes the first prediction
  // faster. Call `dispose` to release the WebGL memory allocated for the return
  // value of `predict`.
  mobilenet.predict(tf.zeros([1, 1, IMAGE_SIZE, IMAGE_SIZE])).dispose();
  ...
  if (catElement.complete && catElement.naturalHeight !== 0) { 
    ...
    procImg = procImage.reshape([1, 224, 224]);
